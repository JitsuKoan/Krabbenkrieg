...
/*:
 * @plugindesc (v5.01 *) Permite customizar o layout de batalha.
 * @author Moghunter
 *
 * @param -> MAIN <<<<<<<<<<<<<<<<<<<<<<<
 * @desc
...
Game_Action.prototype.apply = function(target) {
	 ...
	 if (target.isActor()) {
		 if (oldhp > target.hp) {target._bhud_face_data = [70,0,3,70]}
		 else if (oldhp < target.hp) {target._bhud_face_data = [0,90,1,90]};
	 };
};
...
Game_Action.prototype.prepare = function() {	
	...
	if (this.subject().isActor() && String(Moghunter.bhud_face_zoom) === "true"){this.subject()._bhud_face_data = [0,90,2,90];};
};
...
Battle_Hud.prototype.update_face_zoom = function() {
	if (this._battler._bhud_face_data[1] > 0) {this._battler._bhud_face_data[1] -= 1;
		...
		else if (this._battler._bhud_face_data[1] < 45) {this._face.scale.x -= 0.005;
		         if (this._face.scale.x < 1.00) {this._face.scale.x = 1.00;};	
	    }
		else if (this._battler._bhud_face_data[1] < 90){this._face.scale.x += 0.005;
